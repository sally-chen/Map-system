...
void drawtext_in(const t_bound_box& bbox, const std::string& text, bool map) {
    drawtext(bbox.get_center(), text, bbox, map);
}

void drawtext_in(const t_bound_box& bbox, const std::string& text, float tolerance, bool map) {
    drawtext(bbox.get_center(), text, bbox, tolerance, map);
}

void drawtext(
    const t_point& text_center, const std::string& text, const t_bound_box& bounds, float tolerance, bool map ) {
    ...
    drawtext(text_center, text, tolerance_bounds, map);
}

void drawtext(const t_point& text_center, const std::string& text, const t_bound_box& bounds, bool map) {
...
//        printf("the center of the text \"%s\" isn't in its bounding box", text.c_str());
    ...
    min_bounds *= 2;
    drawtext(text_center, text, min_bounds.x, min_bounds.y, map);
}

void drawtext(const t_point& text_center, const std::string& text, float boundx, float boundy, bool map) {
    drawtext(text_center.x, text_center.y, text, boundx, boundy, map);
}
 ...
 */
void drawtext(float xc, float yc, const std::string& str_text, float boundx, float boundy, bool map) {
    ...
    // their caveats.
    float X11_xmagicoffset;
    float X11_ymagicoffset;
    if (map) {
        X11_xmagicoffset = 0;
        X11_ymagicoffset = 0;
    } else {
        //THIS IS TO LEFT JUSTIFY SHIT
        X11_xmagicoffset = coord_width;
        X11_ymagicoffset = coord_height;
    }
